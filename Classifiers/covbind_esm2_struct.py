...
def train_epoch(model, data_loader, criterion, optimizer, device):
    model.train()
    total_loss = 0.0
    for vh_id,  vl_id, ag_embs, labels in tqdm(data_loader, mininterval=5, desc="Training"):
...
def val_epoch(model, data_loader, criterion, device):
    ...
    total_samples = 0 
    with torch.no_grad():
        for vh_id, vl_id, ag_embs, labels in tqdm(data_loader, mininterval=5, desc="Validating"):
            # Move data to the specified device
            vh_id = vh_id.to(device)
            vl_id = vl_id.to(device)
            ag_embs = ag_embs.to(device) 
...
if __name__=="__main__":
    ...
    # savepath = 'saved_models/covbind_ESM2_Struct_pt.pth'
    # print(savepath)
    ...
    savepath = 'saved_models/covbind_ESM2_Struct_ft.pth'
    print(savepath)
    model = CovBind_ftESM(freeze=False) 
    model = train(model, train_loader, val_loader, num_epochs, 0.00005, device, 
                  savepath=savepath) 
    model.load_state_dict(torch.load(savepath, map_location=device)) 
    test_run(model, data_loader_test, device) 
